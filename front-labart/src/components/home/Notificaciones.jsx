...
const NotificationsPage = () => {
  ...
  const fetchNotifications = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`/notificaciones_usuario/${currentUserId}`);
  ...
  try {
    await axios.put(`/notificaciones/marcar_como_leidas/${currentUserId}`);
    fetchNotifications(); // 🔄 Vuelve a cargar solo las notificaciones
  } catch (error) {
...
const deleteAllNotifications = async () => {
  try {
    await axios.delete(`/notificaciones/eliminar/${currentUserId}`, {
      data: { tipos: [2, 3, 4, 5] } // Asegúrate de que estos tipos coincidan con los de tu base
    });
