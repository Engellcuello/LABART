...
const PublicacionPropiaModal = ({ mostrar, onClose, publicacion }) => {
  ...
  const manejarReaccion = async (idReaccion) => {
    try {
      const usuarioActual = localStorage.getItem('ID_usuario');
      const idPublicacion = publicacion.ID_publicacion; // <- Esto es CLAVE
      ...
      // Quitamos reacción anterior si existe
      if (reaccionUsuario) {
        await quitarReaccion(idPublicacion);
      }
  
      // Creamos la nueva reacción
      await axios.post('http://127.0.0.1:5000/publicacion_reaccion', {
  ...
  const quitarReaccion = async (idPublicacion) => {
    try {
      ...
      // Buscar la reacción del usuario para esta publicación
  ...
  const eliminarPublicacion = async () => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:5000/publicacion/${publicacion.ID_publicacion}`);
      Swal.fire('Eliminada', 'La publicación ha sido eliminada', 'success');
      onClose();
      // Aquí podrías añadir una función para actualizar la lista de publicaciones
    } catch (error) {
  ...
  const guardarCambios = async () => {
    try {
      await axios.put(`http://127.0.0.1:5000/publicacion/${publicacion.ID_publicacion}`, {
  ...
  const cargarEtiquetasDisponibles = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:5000/etiquetas/disponibles');
  ...
  const asignarEtiqueta = async (idPublicacion) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:5000/publicaciones/${idPublicacion}/etiquetas/asignar`,
        {
  ...
  // Guardar los cambios de categorías
  const guardarCategorias = async () => {
    if (!publicacion?.ID_publicacion) return;

    try {
      await axios.delete(`http://127.0.0.1:5000/publicaciones/${publicacion.ID_publicacion}/categorias`);

      for (const idCategoria of categoriasSeleccionadas) {
        await axios.post(`http://127.0.0.1:5000/publicacion_categoria`, {
