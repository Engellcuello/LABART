...
const PasswordResetModal = ({ onClose }) => {
  ...
  const handleEmailSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://127.0.0.1:5000/password-reset/init', { email });
  ...
  const handleOtpSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://127.0.0.1:5000/password-reset/verify', {
        user_id: userId,
        otp
      });
      setStep(3);
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://127.0.0.1:5000/password-reset/complete', {
        user_id: userId,
